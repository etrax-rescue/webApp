...
$(function () {	
		...
		if (confirm("Soll eine neue Ressource angelegt werden?")) {
			$.ajax({
				...
				success: function (msg) {
					$(".modal.feedback").modal('show').find("h5").html("Eine neue Ressource wurde angelegt!<br>" + msg);
					setTimeout(function () {
	...
	if (confirm(confirmtext)) {
		$.ajax({
			...
			success: function (msg) {
				$(".modal.feedback").modal('show').find("h5").html("Die Ressource " + name + " wurde gelöscht!<br>" + msg);
				setTimeout(function () {
					$(".modal.feedback").modal('hide');
					location.reload();
					//window.location.href = window.location.href.replace( /[\&#].*|$/, "?oid="+oid );
