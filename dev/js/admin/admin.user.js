$(function () {	
	...
	$("body").on("click", ".user_import_preview", function () {
		...
		user.forEach((row, index) => {
			...
			$('.previewtable').append(" <div class='card'><div class='card-header' id='heading_" + index +"'><h5 class='mb-0'>" + appendtext_cb + " <a class='btn btn-link collapsed' data-toggle='collapse' href='#collapse_" + index +"' aria-expanded='false' aria-controls='collapse_" + index +"' role='button'>" + user_name +"</a></h5></div><div id='collapse_" + index +"' class='collapse' aria-labelledby='heading_" + index +"' data-parent='#accordion'><div class='card-body'>" + appendtext + "</div></div></div>");
		});
